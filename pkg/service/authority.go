...
func RequireAuthority(repos repository.Repositories, callerId string, rolesAllowed ...string) error {
	ownerId, _ := env.Get("MEMBER_ROLE_OWNER_ID")
	adminId, _ := env.Get("MEMBER_ROLE_ADMIN_ID")
	memberId, _ := env.Get("MEMBER_ROLE_MEMBER_ID")
...
func GetRole(repos repository.Repositories, memberId string) (string, error) {
	...
	ownerRoleId, _ := env.Get("MEMBER_ROLE_OWNER_ID")
	adminRoleId, _ := env.Get("MEMBER_ROLE_ADMIN_ID")
	memberRoleId, _ := env.Get("MEMBER_ROLE_MEMBER_ID")
