...
API=platform-api
SANDBOX_API=sandbox-platform-api
...
build-sandbox: test-envvars
	CGO_ENABLED=0	GOOS=linux GOARCH=amd64 go build -o ./cmd/app/main ./cmd/app/main.go
	docker build --platform linux/amd64 -t $(ECS_SANDBOX_API_REPO):${TAG} cmd/app/
	rm cmd/app/main

push: test-envvars
	aws ecr get-login-password --region $(AWS_REGION) | docker login --username AWS --password-stdin $(ECR)
	docker push $(ECS_API_REPO):${TAG}

push-sandbox: test-envvars
	aws ecr get-login-password --region $(AWS_REGION) | docker login --username AWS --password-stdin $(ECR)
	docker push $(ECS_SANDBOX_API_REPO):${TAG}
